...
public class ProductDao implements IDao<Product, Long> {
	...
	public List<TrendingProductDTO> getTrendingProducts() {
		...
		try {
			while (rs != null && rs.next()) {
				trendingProducts.add(new TrendingProductDTO(rs.getLong("productId"), rs.getLong("categoryId"),
						rs.getString("name"), rs.getDouble("price"), rs.getString("description"), rs.getInt("stock"),
						rs.getString("image")));
			}
